...
public class SimpleDBParserWhereTest {
	...
	public void should_pass_for_complex_where() throws ParseException {
		parser = new SimpleDBParser("select * from `testDB.simpleUser` where not \t (attr5 in ('1','2') or (attr10 <= '1.3' and attr10 > '0')) intersection attr1  > '2.3' and (attr2 like 'asd%' or every(attr3) between '1' and '4')");
		parser.selectQuery();
	}
	
	@Test(expected = ParseException.class)
	public void should_fail_for_malformed_complex_where() throws ParseException {
		parser = new SimpleDBParser("select * from `testDB.simpleUser` where attr1  > '2.3' and attr2 like or every(attr3) between '1' and '4'");
		parser.selectQuery();
	}
