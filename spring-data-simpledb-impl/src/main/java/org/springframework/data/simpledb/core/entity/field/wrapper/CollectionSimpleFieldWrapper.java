...
public class CollectionSimpleFieldWrapper<T, ID extends Serializable> extends AbstractSimpleFieldWrapper<T, ID> {
    ...
    public List<String> serializeValue() {
        ...
        if(getFieldValue() != null) {
            ...
            String fieldMarshaled2JSON = JsonMarshaller.getInstance().marshall(getFieldValue(), genericTypeClass.getName());
    ...
    public Object deserializeValue(List<String> value) {
        ...
        if (value.size() > 0) {
            ...
            jsonCollection = (Collection<?>) JsonMarshaller.getInstance().unmarshallWrapperCollection(fieldValue);
        }
